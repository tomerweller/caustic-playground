...
let camera, renderer, stats, scene, ball, floor, light, ballGeometry, floorGeometry, ambient;
...
const initScene = loadedScene => {
  ...
  ambient = scene.getObjectByName("AmbientLight");
...
const render = () => {
  ...
  const floorRGB = settings.floorColor.map(c => Math.floor(c));
  floor.material.color = new THREE.Color(`rgb(${floorRGB[0]}, ${floorRGB[1]}, ${floorRGB[2]})`);
...
const settings = {
  ...
  calcCaustics: calcCaustics,
  ambientIntensity: 50,
  floorColor: [255,180,0]
};
...
const initSettings = () => {
  ...
  gui.add(settings, 'ambientIntensity', 0, 150);
  gui.addColor(settings, 'floorColor')
  gui.add(settings, 'calcCaustics');
